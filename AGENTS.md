...
## Status
⚠️ YouTube — Vercel serverless function (`/api/yt-extract`) calls InnerTube API from AWS Lambda IP; frontend tries Vercel function first, falls back to Go SSE; direct CDN URL download from browser IP
...
## Key Architecture
- **Vercel API**: `/api/yt-extract?id=VIDEO_ID` — serverless function call InnerTube API dari AWS Lambda IP (tidak diblokir YouTube)
- **SSE**: `/api/stream/video` dan `/api/stream/image` untuk ekstraksi format (fallback)
- **Proxy**: `/api/proxy?url=...&yt_format=...` untuk download aktual
- **Direct CDN**: format dari Vercel API punya `directUrl` — browser download langsung dari googlevideo.com (IP residential, bukan datacenter)
